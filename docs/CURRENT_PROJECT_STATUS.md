...
Receivables Aging milestone: CLOSED using grouped-per-currency semantics. `receivablesAgingReport` aggregates positive persisted schedule `balance_amount` by Agreement currency and backend date buckets, scoped by schedule organization/branch and protected by `agreement.payment_schedules.view`. Cancelled, waived, and zero-balance schedules are excluded; unapplied advances/overpayments are not netted. `/reports` renders independent currency sections with no FX or cross-currency total. Reports remains PARTIAL pending historical trends and exports.
