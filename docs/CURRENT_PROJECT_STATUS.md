...
Tenant legacy migration milestone: PARTIAL. Production connectivity is now verified for `srv1856.hstgr.io` / `u249550001_bm_ai_hub` and the environment is `production`. Read-only inspection shows `tenants` exists with 25 rows, but migration `2026_08_20_000000_add_branch_id_to_tenants_table` is still pending, so production has no `branch_id` column yet. No migration or backfill was run because approved backup/rollback protection was not independently verified. Operator action: verify a restorable production backup/snapshot, then deploy the nullable branch migration before classification.
