...
Business-approved synthetic allocation milestone: PARTIAL. Production Branches 1 (`FUJ`) and 2 (`DXB`) both belong to Organization 1; 25 Tenants remain branch-null and have zero Agreements. A deterministic dry-run using seed `BM_TENANT_BRANCH_SPLIT_V1` produced 13 assignments to Branch 1 and 12 to Branch 2. No values were applied because production backup/rollback protection remains unverified. Owners have no direct `branch_id`; Owner allocation is NOT APPLICABLE.
