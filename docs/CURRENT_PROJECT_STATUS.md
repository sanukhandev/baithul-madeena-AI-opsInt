...
Tenant legacy migration milestone: PARTIAL. The configured deployment target is production MySQL `srv1856.hstgr.io` / `u249550001_bm_ai_hub`, but the current execution environment cannot open the MySQL socket (`SQLSTATE[HY000] [2002]`). No deployment rows were read or modified, no mappings were applied, and `branch_id` remains nullable. NOT NULL enforcement is intentionally blocked until classification and remediation run against the verified database.
