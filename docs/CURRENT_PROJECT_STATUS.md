...
Owner Summary milestone: CLOSED. Owner branch ownership is not represented by `owners.branch_id`; the authoritative safe report is organization-wide over non-deleted Owners and persisted `Owner.status`, protected by `owners.view`. `/reports` labels the summary organization-wide and uses an organization-partitioned query key. OwnerFeatureTest now supplies the established organization request header centrally, while an explicit missing-context test preserves fail-closed behavior. Overall Reports remains PARTIAL pending aging, historical trends, and exports.

Receivables aging discovery: PARTIAL/BLOCKED. Payment schedules provide authoritative `total_amount`, `paid_amount`, `balance_amount`, `due_date`, persisted statuses, and branch/organization scope; payment allocations update schedule balances, while unapplied advances/overpayments remain separate. However, Agreement currency is a free-form per-agreement value with no organization/branch single-currency invariant or FX conversion contract. A singular cross-currency aging total would therefore be unsafe. No aging API or UI was added.
