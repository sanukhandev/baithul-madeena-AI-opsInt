...
Agreement Status Summary milestone: CLOSED. `agreementReportSummary` provides complete current-state status/type counts from scoped Agreements, including Cancelled/Renewed and explicit other-type reconciliation; `/reports` displays the live summary.
...
Business-approved synthetic allocation milestone: CLOSED for controlled development data. Branches 1 (`FUJ`) and 2 (`DXB`) belong to Organization 1; all 25 unmapped Tenants were allocated deterministically with seed `BM_TENANT_BRANCH_SPLIT_V1` (13 to Branch 1, 12 to Branch 2). This is a development synthetic allocation, not historical ownership inference. Tenant integrity checks pass, and migration `2026_08_29_000000_make_tenant_branch_id_not_nullable` now enforces `tenants.branch_id NOT NULL` while preserving the foreign key and `idx_tenants_branch_status`. Owners now have direct non-null `branch_id` ownership under the same operational architecture.
...
Tenant Summary milestone: CLOSED. The typed `tenantReportSummary` query aggregates persisted Tenant statuses (`Prospect`, `Active`, `Inactive`, `Blacklisted`) server-side from direct `tenants.branch_id` scope under `tenants.view`; `/reports` renders the branch-partitioned live summary with loading, error, and zero states. No Agreement-derived status or branch logic is used.
