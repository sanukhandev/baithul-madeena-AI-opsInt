...
Tenant legacy migration milestone: PARTIAL. DNS resolves `srv1856.hstgr.io` to Hostinger infrastructure and ICMP responds, but TCP port 3306 fails on both resolved IPv4/IPv6 endpoints; Laravel reports `SQLSTATE[HY000] [2002]`. This indicates remote MySQL access is unavailable from the current runtime (hosting firewall/Remote MySQL allowlist or egress policy), not an evidenced credential failure. No deployment rows were read or modified, no mappings were applied, and `branch_id` remains nullable. Operator action: enable Remote MySQL for the runtime source IP or provide the approved reachable Hostinger endpoint/allowlist. NOT NULL enforcement remains blocked.
