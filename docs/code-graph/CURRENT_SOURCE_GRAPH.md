...
The frontend has a small plain-`fetch` GraphQL client plus `src/lib/erp.ts` TanStack Query hooks. Properties, Agreement payments, Property → Building → Unit, and Tenant → Agreement views consume authenticated backend data; remaining operational routes are not yet wired.
